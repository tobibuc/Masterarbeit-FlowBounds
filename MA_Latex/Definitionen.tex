...
Integer Programs") for solving our problem. For details of the solvers setup setup we refer to the PhD Thesis of Tobias 
Achterberg \cite{Achterberg2007} where he describes the main design of his Constraint Integer Programming solver SCIP. 
The following definitions also follow this work.

\begin{definition}
 A \textit{constraint program} is a triple $(\mathcal{C},\mathcal{D},f)$ and consists of solving
 $$f^*=\min{f(x)|x\in \mathcal{D}, \mathcal{C}(x)}$$ 
 where $\mathcal{D}=\mathcal{D}_1\dots \mathcal{D}_n$ is the domain set, $f:\mathcal{D}\to\R$ the objective function and
