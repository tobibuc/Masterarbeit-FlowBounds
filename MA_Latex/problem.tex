...
\begin{definition}
Given a feasible network flow $f:A\to \R$ in a network $G=(V,A)$. Let us say $f$ can be \textit{divided} or we 
\textit{divide the network flow} $f$ into flows $f_1$ and $f_2$ if there exist flows $f_1, f_2:A\to \R$ such that 
\begin{enumerate}
