...
% \usepackage{svg}
\usepackage{svg, pstricks, pdfpages, times, graphicx, currvita, hyperref, longtable, float, caption, subcaption}
\usepackage{amsmath,amsfonts,amssymb, amsthm}%, dsfont, bbm, mathtools, stmaryrd}

%the next lines ensure that ALL page numbers are at the bottom of the page
\usepackage{fancyhdr}
\fancyhf{}
\cfoot{\thepage}
\pagestyle{fancy}    
...
\tikzset{every picture/.append style={scale=2.6
}}%maybe better set locally?
...
\tableofcontents
\newpage
\chapter*{Zusammenfassung}\input{deutschZusammenfassung}
\newpage
\chapter{Introduction} \input{Einleitung}
\newpage
% \input{motivation}
% \newpage
\section{Basic Notation and Definitions}\input{Definitionen}
\newpage
\chapter{The Acyclic Flowbound Problem}\input{problem}
\section{Computational Complexity}\input{complexity}
\newpage
\chapter{Models and Solving Approaches}\input{models}
\newpage
\chapter{Implementation and Computational Results}\input{compresults}
\chapter{Conclusion}\input{conclusion}
\chapter{Appendix}\input{appendix}
% \newpage
% \section{Summary}
