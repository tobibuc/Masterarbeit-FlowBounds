...
augmenting paths in the network.%TODO funktioniert das, oder gleich sagen nur als heuristik?
...
The main idea in this chapter is to design an algorithm that uses path augmentation to saturate the given flow demands. 
...
\label{algo:pathHeur}
 ...
 \begin{algorithmic}[5]%[0] means no line numbers, n means every nth line number is displayed
  \Function{Heuristic}{$G=(V,A),\, e=(u,v)\in A,\, b:V\to \R $}%hier übergabeparameter
	...
	\State $P\gets $\Call{acyclicPathOverE}{$v, e$}
...
%       \Else
	...
	\State $P\gets$\Call{acyclicPath}{$s$}\Comment{try finding a path that avoids $e$}
	\State $c(e)\gets c_{alt}(e)$
	\If{$P\ne \emptyset$}
	  \State \Call {maxAugment}{$P$}\label{heur:lineAugCase2}
	  \State \textbf{continue}
	\Else
	  \State $P\gets$ \Call{acyclicPath}{$s$}
...
% 
% \begin{prop}
%  Given a network $G=(V,A)$ and an acyclic flow $f:A\to \R$ with $f(e)$ maximum and flow balances $b:V\to \R | 
% \sum_{v\in V}b(v)=0$ . For $x\in \R$ define flow balances $b':V\to \R$ as $b'(s)=b(s)-x, s\in V$, $ b'(t)=b(t)+x, t\in 
% V, t\neq s$, $ b'(v)=v\forall v\in V$.\\
% 
% There is an acyclic flow $f':A\to\R$ for the balances $b'$ with $f'(e)=f(e)+x$ if and only if there is an augmentable 
% path from $s$ to $t$ using $e$ whose augmentation does not cause cyclic flow.
% \end{prop}
% In other words this proposition says that the described path-augmentation based algorithm works right as long as the 
% flow on $e$ can  be increased. If this is not the case the algorithm tries to find any acyclic path. 


\begin{prop}
 The described path augmenting algorithm finds the right solution for the acyclic flowbound problem. 
\end{prop}
\begin{proof}
  We show the equivalence of having an acyclic flow in a network and the ability to find a 
 ...
 We have to show that the selection process of our algorithm is sufficient for maximality. There are three points in 
 ...
 If we try to find a path to augment we make some kind of local decision. We have to prove that this is globally 
 optimal. This means we have to show that if we do not find any acyclic path at a point in the algorithm then there is 
no acyclic flow $f$ with higher flow value $f(e)$ on $e$ at all. 
...
%es koennte noetig sien dass ich nicht einfache pfade benötige, um kreisfreien fluss zu ermoeglichen (vor allem 
% augmentierung in der gleichen richtung zwei mal)
 
 
%  We have to show that if we did not find an augmentable path increasing flow over $e$ in the network, it is optimal to 
% choose a path that does not increase flow over $e$ - and if this does not work either a path that may decrease the flow 
% over $e$. 
%  
