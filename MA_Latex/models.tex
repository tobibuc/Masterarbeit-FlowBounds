...
\cite{scip}, \cite{Achterberg2009} etc. Often there are different possible 
...
This model still allows cyclic flow, so we have to find constraints to avoid cycling. 
...
\subsection{Model 2: Acyclicity Constraints On All Cycles}
...
\draw[arc] (a) -- (b)node [pos=0.5, left]{$e_1$};
\draw[arc] (b) -- (c)node [pos=0.5, right]{$e_5$};
\draw[arc] (c) -- (a)node [pos=0.5, left]{$e_4$};
\draw[arc] (c) -- (d)node [pos=0.5, right]{$e_3$};
\draw[arc] (d) -- (b)node [pos=0.5,right]{$e_2$};
\draw[ultra thick, blue, rounded corners](0.1,0)--(0.94,0.85)--(0.94,-0.85) --cycle;
\draw[ultra thick, blue, rounded corners](2-0.1,0)--(2-0.94,0.85)--(2-0.94,-0.85) --cycle;
...
\end{tikzpicture}
\caption{The example tetrahedron graph with the blue marked cycles whose acyclicity constraints can not forbid cyclic 
...
\node (6) at (1.5,0.1)[source]{-1};
\node (7) at (1.8,-1)[vertex]{};
\node (8) at (1.9,1.5)[vertex]{};
\node (9) at (2,0)[vertex]{};
\node (t) at (2.5,0)[sink]{2};
\draw[ultra thick, blue!30, rounded corners](0.6,1)--(1,0.3)--(1.05,0.55)--(1.8,1.42) --cycle;
\draw[ultra thick, blue!30, rounded corners](1.1,0.2)--(1.1,0.4)--(1.4,0.1)--(1.3, -0.6) --cycle;
\draw[ultra thick, blue!30, rounded corners](1.15,0.53)--(1.5,0.15)--(1.84,1.4) --cycle;
\draw[ultra thick, blue!30, rounded corners](0.5,0.9)--(0.9,0.2)--(1.2,-0.6)--(0.1,0) --cycle;
...
% \draw[ultra thick, blue, rounded corners](-.2,0)--(0.45, 1.1)--(1.95,1.6)--(2.7,0)--(1.9,-1.1)--(1.8,-1.1)--(0.75,-1.1) 

\draw[ultra thick,red, rounded corners](-0.1,0)--(0.45,1.1)--(2,1.6)--(1.55,0.1)--(1.35,-0.8) --cycle;%s-1-8-6-5-s
% \draw[arcflow] (s)--(3)--(t);
...
\draw[edge] (5)--(6);
\end{tikzpicture}
\caption{This picture shows one example how the cycle $C$ (marked red) defines a region $G'$ of the graph where it is 
enough to have acyclicity constraints on a small set of cycles (marked light blue). Thus the acyclicity constraint of 
$C$ does not have to be added to the MIP. But we cannot extend this to the whole graph $G$, because the source 
node in the middle must not be in the interior $G'$.}
 \label{bild:redundantCycle}
\end{figure}
  
\begin{remark}
  The power of this proposition is that it can be applied recursively to eventually form a region in the graph 
  where only the smallest cycles need acyclicity constraints. For an example see figure \ref{bild:redundantCycle}.
\end{remark}
...
\newpage
\section{The Constrained Path Augmentation Model}%TODO kann man so ein modell finden, das mithilfe von constrained 
...
\draw[edge] (t)--(d){};
\end{tikzpicture}
\caption{The optimum on the example graph from above figure \ref{bild:blockingPathflowNet} - all flow is going over $e$ 
while the flow is still acyclic in the whole network. The maximum flow value on $e$ is 2 in this example.}
...
\begin{algorithm}
 \caption{path based heuristic flow bound algorithm for the upper bound}
\label{algo:pathHeur}
 ...
 \begin{algorithmic}[5]%[0] means no line numbers, n means every nth line number is displayed
  \Function{HeurMaxAcycFlow}{$G=(V,A),\, e=(u,v)\in A,\, b:V\to \R^{|V|} $}%hier übergabeparameter
    \State define $b':V\to \R,\, b'(v)\gets 0 \quad\forall v\in V$ \Comment{temporary balances}
    \State define $f: A\to\R,\, f(a)\gets 0 \quad\forall a\in A$
    \While {$\neg (b+b'\equiv 0) $}\Comment{while there are active source-sink-pairs}
      \If{ $c_u(e)>f(e)$}\Comment{assume we maximize $e$ in forward direction}
      \State choose active source $s\in V$ 
	\State $P\gets $\Call{acyclicEdgeProgOverE}{$s, e$}
...
%       \Else
	...
	\State $c_u^{old}(e)\gets c_u(e)$, $c_l^{old}(e)\gets c_l(e)$\Comment{save old capacity values}
	\State $c_u(e)\gets f(e),\, c_l(e)\gets f(e)$\Comment{make $e$ blocked for a short time}
	\State $P\gets$\Call{acyclicEdgeProg}{$s$}\Comment{try finding a path that avoids $e$}
	\State $c_u(e)\gets c_u^{old}(e), c_l(e)\gets c_l^{old}(e)$
	\If{$P\ne \emptyset$}
...
So we have seen that augmentation algorithms would only work with a too powerful algorithm that shifts flow before 
