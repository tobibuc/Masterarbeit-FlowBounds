 ...
 \begin{algorithmic}
  \Function {maxAugment}{$P$}
    ...
    \ForAll{$a\in P$}
      ...
      \If{$a$ used twice in the same direction in $P$}
...
\begin{proof}
  ...
  \begin{itemize}
   ...
   \item[Inductive Step:] Assume $f_{b_{i-1}'}'(e)\le f_{i-1}(e)$ for all feasible $f'_{b_{i-1}'}$. Now in step $i$ the 
      ...
      \begin{enumerate}
       \item Case: (Forward augmentation on $e$, alg. line \ref{heur:lineCase1}) In the first case we found a path 
	  ...
	  which is clearly a contradiction. \Lightning
       \item Case: (Cannot increase flow over $e$ but go around, alg. line \ref{heur:lineAugCase2}) 
