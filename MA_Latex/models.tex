...
are well known and belong to the standard flow algorithms. The idea was first described by ...  TODO%TODO Zitate 

The advantage of this approach is clear: it is very easy to compute a path from one node to another or decide that 
there is none. It can essentially be done by simple graph search algorithms. If we want to maximize flow over arc 
$a=(u,v)$ we just look for a path from our source $s$ to node $u$ and one from node $v$ to a sink $t$. Of course: if we 
combine these two, this could produce cyclic flow. %TODO BILD

But on every path flow is going over arc $a$ only once, while in a MIP formulation as normal Min-Cost Flow Problem it 
could be as much as the capacity of $a$ allows. And this heuristic is still a valid upper bound on the flow on $a$.
%TODO prove it