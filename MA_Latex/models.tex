...
worse: Figure \ref{bild:backaugmentationIsNecessary} %TODO make this picture, where we have to go back to saturate 
...
sum of flow from all sources together. We will also analyze its worst-case running time.
...
\label{algo:pathHeur}
 ...
 \begin{algorithmic}[0]%[0] means no line numbers, n means every nth line number is displayed
...
completing it.
\begin{itemize}
 \item [First Stage:] In step $i$ there is an additional supply and demand of say $\alpha_i$ satisfied. In each of the 
steps the flow value $f_i(e)$ is also increased by $\alpha_i$, so while the algorithm is in this stage it has all flow 
going over $e$. In any acyclic flow this can be not more: if it was more in a flow $f'_i(e)$ the flow from one source 
has to go over $e$ twice. But this would form a cyclic flow somewhere. \Lightning.
