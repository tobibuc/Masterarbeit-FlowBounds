...
\begin{algorithm}
 ...
 \begin{algorithmic}[0]%[0] means no line numbers, 1 means each line number
  \Function{Heuristic}{$G=(V,A),\, e=(u,v)\in A,\, b:V\to \R $}%hier übergabeparameter
    ...
    \While {$\neg (b+b'\equiv 0) $}
      \If{ $c(e)>f(e)$}
	\State $P_1= pathFromSource(u)$
	\State $P_2= pathToSink(v)$
	\If{$P_1\ne \emptyset \land P_2\ne \emptyset$}
	  \State $P=P_1+e+P_2$
	  \State $maxAugment(P)$
	  \State continue
	\EndIf
      \Else
	\State choose $v\in V$ with $b(v)+b'(v)<0$
	\State $c_{alt}(e)\gets c(e)$
	\State $c_(e)\gets f(e)$
	\State $P=pathToSink(v)$
